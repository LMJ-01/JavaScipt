...
// 기존 요소에 값을 3을 곱하고, 인덱스를 더한 값을 새로운 배열에 저장.
...
// 정수 배열에서 80점 이상만 골라서 highScore 배열을 만들기
let scores = [90, 35, 77, 85, 92, 45]
highScore = scores.filter(score => score >= 85)
console.log("highScore : ", highScore);

// 80점 이상인 값과 인덱스를 함꼐 찾고 싶다.
let highScore2 = scores.filter((score, index) => {
    if (score > 80) {
        console.log(`index : ${index}, score L ${score}`)
        return score;
    }
});
